...
const Overview = ({handeOnSelect, handleCreateWorkspace, handleNewChat, page, chats, chatId, handleDetailView}: Props) => {
  ...
  return (
    <>
        <Command.Group className='mb-6'>
                <small>Chats</small>
                {chatExists(chats, chatId as string) && <ListItem onSelect={() => {
                    const chat = chats.filter(chat => chat.id === chatId)[0];
                    if (chat) {
                        handleDetailView(chat);
                    }
                }}><ArrowRight className='w-4 h-4' />Jump to active chat</ListItem>}
